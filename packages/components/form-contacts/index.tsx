...
const FormContacts: React.FC = () => {
  ...
  function onSubmit() {
    return handleSubmit(async (data) => {
      ...
      return axios
        .post('/api/telegram/contact-message', {
          name: data.name,
          phone: data.phone,
          email: data.email,
  ...
  return (
    <>
      <Modal open={openModal} setOpen={setOpenModal}>
        <div className="px-8 py-6 space-y-2 flex flex-col rounded-3xl">
          <h3>{t(error ? 'modal.error.title' : 'modal.confirm.title')}</h3>
          <p className="mt-2 text-sm">
            {t(error ? 'modal.error.body' : 'modal.confirm.body')}
          </p>
          <button
